...
export const store = {
  ...
  submitHousePurchase: async (houseId: number, username: string): Promise<boolean> => {
    const { data: house } = await supabase.from("houses").select("name, price").eq("id", houseId).single();
    const h = house as Record<string, unknown> | null;
    const { error } = await supabase.from("house_purchase_requests").insert({
      house_id: houseId, username,
      house_name: h?.name || "",
      house_price: h?.price || 0,
      status: "pending",
    });
    return !error;
  },
  ...
  submitLicense: async (username: string, licenseType: string, plateNumber?: string) => {
    await supabase.from("license_applications").insert({
