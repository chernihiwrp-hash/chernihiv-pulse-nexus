...
type Idea = { id: number; author: string; text: string; type: "idea" | "petition"; likes: number; dislikes: number; status: "active" | "approved" | "rejected" };
...
const initialIdeas: Idea[] = [
  ...
  { id: 3, author: "Player_03", text: "Організувати щотижневі RP івенти", type: "idea", likes: 22, dislikes: 0, status: "active" },
];
...
const CityVoice = () => {
  ...
  const handleLike = (id: number) => setIdeas(prev => prev.map(i => i.id === id ? { ...i, likes: i.likes + 1 } : i));
  const handleDislike = (id: number) => setIdeas(prev => prev.map(i => i.id === id ? { ...i, dislikes: i.dislikes + 1 } : i));

  const submit = () => {
    if (!message.trim()) return toast.error("Напишіть повідомлення");
    setIdeas(prev => [{ id: Date.now(), author: "Ви", text: message, type, likes: 0, dislikes: 0, status: "active" }, ...prev]);
    ...
    <div className="min-h-screen bg-background pb-20 px-4 pt-4">
      ...
      <div className="liquid-glass-card rounded-2xl p-4 mb-4 animate-fade-in">
        ...
        <textarea
          ...
          placeholder={type === "idea" ? "Ваша ідея..." : "Текст петиції..."}
            ...
            <NeonCard glowColor="green">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <button onClick={() => handleLike(idea.id)} className="flex items-center gap-1 text-[10px] text-primary hover:text-primary/80 transition-colors active:scale-95">
                    <ThumbsUp className="w-3.5 h-3.5" /> {idea.likes}
                  </button>
                  <button onClick={() => handleDislike(idea.id)} className="flex items-center gap-1 text-[10px] text-destructive hover:text-destructive/80 transition-colors active:scale-95">
