...
import { Gift, Clock, Zap, Trophy, Star, Flame, Palette, Check, Lock } from "lucide-react";
...
export interface Theme {
  ...
  bgGradient: string; // bottom background glow
  description: string;
}
...
const Shop = () => {
  ...
  const [activeTab, setActiveTab] = useState<"rewards" | "themes">("themes");
  ...
  const buyTheme = (theme: Theme) => {
    ...
    const newBalance = getBalance(nick);
    ...
    <div className="min-h-screen pb-20 px-4 pt-4">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Нагороди та теми</p>
        </div>
      ...
      <div className="flex gap-2 mb-4 liquid-glass rounded-2xl p-1">
        {[
          { id: "themes" as const, label: "🎨 Теми", icon: Palette },
          { id: "rewards" as const, label: "🎁 Нагороди", icon: Gift },
        ].map(tab => (
      ...
      {activeTab === "themes" && (
        <div className="space-y-3 animate-fade-in">
          <p className="text-[10px] text-muted-foreground text-center mb-2">Теми змінюють акцентний колір усього інтерфейсу</p>
          {THEMES.map((theme, i) => {
                    ...
                    <div className="shrink-0">
                      {theme.price === 0 || isOwned ? (
                        ...
                        <button
                          ...
                          style={{
                            background: balance >= theme.price
                              ? theme.preview
                              : "hsl(0 0% 100% / 0.06)",
                        ...
                        >
                          {balance < theme.price ? <Lock className="w-3 h-3" /> : <Zap className="w-3 h-3" />}
                          {theme.price} CR
                        </button>
                      )}
                    </div>
          ...
          <div className="rounded-2xl overflow-hidden"
            ...
            <div className="p-6 text-center">
              <div className="w-20 h-20 rounded-2xl mx-auto mb-4 flex items-center justify-center relative"
                style={{ background: "linear-gradient(135deg, hsl(var(--primary) / 0.2), hsl(var(--secondary) / 0.1))", border: "1px solid hsl(var(--primary) / 0.3)", boxShadow: "0 0 30px hsl(var(--primary) / 0.2)" }}>
              ...
              <div className="flex items-center justify-center gap-2 mb-5">
                ...
                <span className="text-3xl font-black text-primary" style={{ textShadow: "0 0 20px hsl(var(--primary) / 0.6)" }}>
              ...
              {canClaim ? (
                <GradientButton variant="green" className="w-full text-base py-3" onClick={claimReward} disabled={loading}>
                  {loading ? <><Clock className="w-5 h-5 inline mr-2 animate-spin" />Нараховую...</> : <><Gift className="w-5 h-5 inline mr-2" />Забрати нагороду</>}
                </GradientButton>
              ) : (
