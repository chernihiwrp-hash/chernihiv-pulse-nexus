...
const Shop = () => {
  ...
  return (
    <div className="min-h-screen bg-background pb-20 px-4 pt-4">
      <h1 className="font-display text-xl font-bold tracking-wider neon-text-green mb-2">МАГАЗИН</h1>
          ...
          <div key={h.id} className="animate-slide-up" style={{ animationDelay: `${i * 80}ms` }}>
            <NeonCard glowColor="yellow" onClick={() => navigate(`/houses/${h.id}`)}>
              ...
              <div className="flex items-center justify-between mt-3">
                ...
                <span className="text-[10px] bg-neon-yellow/10 text-neon-yellow border border-neon-yellow/20 px-3 py-1.5 rounded-lg font-medium">Детальніше →</span>
              </div>
            </NeonCard>
          </div>
        ))}
